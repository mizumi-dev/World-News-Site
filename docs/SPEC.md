...
MVPでは **①+② を採用**する。追加コストがゼロで、通信社の配信記事という最も多いケースを捕捉できるため。③以降は効果を測ってから判断する。
...
**表示上の扱い**: 重複を単に捨てるのではなく、クラスタの代表1件をカードとして表示し、出典欄に「他N媒体も報道」と添える。まとめサイトとしてはむしろ「何社が報じたか」が情報価値になるため。代表記事は発行時刻が最も古いもの（一次情報に近い）を選ぶ。
...
- ②の類似度の閾値をいくつにするか（低すぎると別ニュースを誤って統合する）
- 日本語・英語など言語をまたいだ同一ニュースは①②では捕捉できない。③の埋め込みを入れるまでは対象外とする
